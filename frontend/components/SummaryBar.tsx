  ...
  return (
    <div className="bg-slate-900 border-b border-slate-800 p-2 overflow-x-auto whitespace-nowrap scrollbar-hide flex gap-3 shadow-lg z-50 shrink-0">
      {TEAMS.map(t => {
        ...
        const isPresent = Object.values(room.players).some((p: any) => p.teamId === t.id);
        const osCount = state.squad.filter(p => p.isOverseas).length;
        return (
          <div key={t.id} className={`bg-slate-800 px-4 py-2 rounded-xl border ${isPresent ? 'border-indigo-500/50' : 'border-slate-700 opacity-50'} min-w-[150px]`}>
